...
from models.backward_quant import *
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m,Conv2dQ):
    ...
    def show_params(self):
        for m in self.modules():
            if isinstance(m, Conv2dQ):
                m.show_params()
    def cosine_distance(self):
        cd_list = torch.tensor(0.0)
        cnt = 0
        a =torch.tensor(0.0)
        for m in self.modules():
            if isinstance(m,Conv2dQ):
                cd_list = (m.cd())
                a = a + torch.mean(cd_list)
                cnt += 1
