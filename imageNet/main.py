...
def main_worker(gpu, ngpus_per_node, args):
    ...
    path='/mnt/mydisk/dataset/Imagenet'
    traindir = os.path.join(path, 'Imagenet_train')
    valdir = os.path.join(path, 'imagenet_val_pytorch/imagenet_val')
...
#            transforms.Resize(256),
    ...
    wandb.init(project="DATE")
    wandb.config.update(args)
    wandb.watch(model,criterion,log="all",log_freq=100)
#	wandb.watch(model,log='gradients')
    count = torch.tensor(-0.005)
    pre_cd = torch.tensor(0.)
    crr_cd = torch.tensor(1.)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if ((epoch%2) ==0 ) and(epoch !=0):
            crr_cd = model.module.cosine_distance()
            print("pre_cd:{}".format(pre_cd))
            print("crr_cd:{}".format(crr_cd))

            for key in state.keys():
                if key.endswith('alpha'):
                    if (pre_cd.cuda() > crr_cd.cuda()) :
                        state[key] = state[key] - count
                        print("alpha(-):{}".format(state[key]))
                    else:
                        state[key] = state[key] + count
                        print("alpha(+):{}".format(state[key]))
...
def save_checkpoint(state, is_best, filename='checkpoints.pth.tar'):
    ...
    if is_best:
#       torch.save(state,'./weights/mobilenetv1_baseline.pth')
        shutil.copyfile(filename, 'mobilenetv1_baseline1.pth.tar')
